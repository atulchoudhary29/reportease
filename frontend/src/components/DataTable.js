...
import logo from '../img/logo_image.png';
...
const DataTableComponent = () => {
  ...
  useEffect(() => {
    setSelectedRows(prevSelectedRows => {
      return prevSelectedRows.map(row => {
        const updatedRow = incidents.find(incident => incident._id === row._id);
  ...
  const handleSearchChange = (event) => {
    ...
    if (searchValue) {
      const searchResults = incidents.filter((incident) =>
        incident.incidentNumber.toLowerCase().includes(searchValue.toLowerCase())
      );
 ...
 // *********************Added this new whole exportToExcel *****************************
 const exportToExcel = async (rowsToExport, fileName) => {
  ...
  const fiberNodeColumnIndex = columns.findIndex(column => column.name === 'Fiber Nodes (Type)');
  ...
  const ExportSelectedButton = () => {
    ...
    return (
      <button
        className={`datatable-buttons${isDisabled ? " datatable-buttons-disabled" : ""}`}
        disabled={isDisabled}
  ...
  const getRowHtml = row => {
    ...
    .map(
      ([type, nodes]) =>
        `<strong style="text-decoration: underline;">${type}</strong><br>${nodes.join('<br>')}`)
    .join('<br><br>');
  
    // *********************line 239 *****************************
    return `
      <div style="font-family: Arial, sans-serif; font-size: 12px; padding: 10px; line-height: 1.5; width: 100%;">
        <img src="${logo}" alt="Your Logo" style="width: 50px; height: 50px; object-fit: contain;">
        <div style="height: 5px; background-color: rgba(0, 0, 255, 0.5); border-radius: 5px;"></div>
        <div style="font-size: 20px; font-weight: bold; margin-bottom: 5px;">Your Text Here</div>
          <table style="border-collapse: collapse; width: 100%; border: 1px solid black;">
            <tr>
              <td style="border-bottom: 1px solid black;"><strong>Start Date and Time:</strong></td>
              <td style="width: 50px; text-align: center; border-left: 1px solid black; border-bottom: 1px solid black;">${row.startDate ? new Date(row.startDate).toLocaleDateString('en-GB').replace(/\//g, '-') : ''}</td>
              <td style="text-align: center; border-left: 1px solid black; border-bottom: 1px solid black;">${row.startTime} GMT +3 </td>              
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;"><strong>Area:</strong></td>
              <td colspan="2" style="text-align: center; border-left: 1px solid black; border-bottom: 1px solid black;">Fibre cut in ${row.area} area</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;"><strong>Nodes:</strong></td>
              <td colspan="2" style="border-left: 1px solid black; border-bottom: 1px solid black;">${formattedFiberNodes}</td>
            </tr>
            <tr>
              <td style="border-bottom: 1px solid black;"><strong>Impact:</strong></td>
              <td colspan="2" style="text-align: center; border-left: 1px solid black; border-bottom: 1px solid black;">${row.serviceImpact}</td>
            </tr>
            <tr>
              <td style="border-bottom: none;"><strong>Informed:</strong></td>
              <td style="width: 50px; text-align: center; border-left: 1px solid black; border-bottom: none;">${row.informedBy}</td>
              <td style="text-align: center; border-left: 1px solid black; border-bottom: none;">${row.informedTime}</td>
            </tr>
          </table>
  ...
  new Promise(async resolve => {
    ...
    newWindow.document.write('<!DOCTYPE html><html><head><style>body {margin: 0;}</style></head><body>');
      ...
      <div className="edit-modal">
        <div className="edit-modal-content form-main">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="input">
              <label htmlFor="notificationNumber" className="input-label-table">
                Notification #:
              </label>
              <input
                type="text"
            ...
            <div className="input">
              ...
              {formData.fiberNodes.map((node, index) => (
                <div key={index}>
                  ...
                  <button type="button" className="input-button delete-node-button" onClick={() => handleDeleteNode(index)}>Delete Node</button>
                </div>
              ))}
            ...
            <div className="input">
              ...
              <select
                ...
                onChange={event => handleNestedChange('serviceImpact', event)}
              >
                <option value="">Select an option</option>
                <option value="X">X</option>
                <option value="Y">Y</option>
            ...
            <div className="input">
              ...
              <select
                ...
                onChange={event => handleNestedChange('informedTeams', event)}
              >
                <option value="">Select an option</option>
                <option value="X">X</option>
                <option value="Y">Y</option>
  ...
  const columns = [
    {
      ...
      header: column => (
        <div className="incidentNumber-header" style={{ textAlign: 'center' }}>{column.name}</div>
      ),
    },
    {
    ...
    // ************** New Start Date ***************
    {
      ...
      cell: row => (
        <div className="startDate-cell" id={`row-${row._id}`}>
          {row.startDate ? new Date(row.startDate).toLocaleDateString('en-GB').replace(/\//g, '-') : ''}
        </div>
      ),
    ...
    // ************** New End Date ***************
    {
      ...
      cell: row => (
        <div className="endDate-cell" id={`row-${row._id}`}>
          {row.endDate ? new Date(row.endDate).toLocaleDateString('en-GB').replace(/\//g, '-') : ''}
        </div>
      ),
    ...
    },
    {
      ...
      header: column => <div className="informedBy-header">{column.name}</div>,
    },
    {
      name: 'Informed Time',
    ...
    // ************** New Fibre Nodes ***************
    {
      ...
      header: column => <div className="fiberNodes-header">{column.name}</div>,
    },
    {
      name: 'Area',
